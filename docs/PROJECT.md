...
| Внимание учительницы | `Gameplay/Teacher`: сущность `Teacher` (`TeacherAttentionComponent`, `TeacherAttentionTimeLeft`, `AlmostCaughtCount`), enum `TeacherAttention`, `ITeacherFactory`, `TeacherExtensions` (`SwitchAttention`/`ReturnToWriting`); `TeacherFeature` в `GameplayCoreFeature` между `NeighboursFeature` и `ExamFeature` |
| Системы учительницы | `InitializeTeacherSystem`, `ScheduleTeacherCheckSystem`, `TickTeacherAttentionSystem`, `AlertTeacherOnMeowSystem`, `AlertTeacherOnPencilSnapSystem`, `ExtendTeacherLookOnMeowSystem`, `TelegraphTeacherTurnSystem`, `WatchClassSystem`, `KeepStaringWhileLeaningSystem`, `ReturnTeacherToWritingSystem` (`Gameplay/Teacher/Systems`, порядок значим — D‑48) |
