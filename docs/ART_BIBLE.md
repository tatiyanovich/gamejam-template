...
| Роль | Шрифт | Источник |
|---|---|---|
| Заголовки, логотип, HUD‑цифры | **Luckiest Guy** | уже в проекте (`Content/UI/Fonts`) |
| Листы, штампы, реплики учительницы | **Patrick Hand** | Google Fonts, OFL — скачать в `Content/UI/Fonts`, сделать TMP‑ассет |
| Текст UI, статы, лидерборд | **Nunito Bold** | Google Fonts, OFL |
