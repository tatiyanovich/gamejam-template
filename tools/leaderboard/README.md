...
- `top` — лучший результат каждого точного ника, отсортированный по `answers` ↓, затем `timeSeconds` ↑ (`GDD §12`);
  длина — параметр `top` (по умолчанию 10, максимум 50).
- `rank` — позиция лучшего результата ника, начиная с 1; **0 означает «ник не найден»** (чужой запрос).
- `total` — количество уникальных ников в лидерборде; все попытки продолжают храниться на листе `scores`.
...
Гоняет `Code.gs` под заглушками `SpreadsheetApp`/`LockService`/`ContentService`: сортировка, лучший результат
на ник, ранг, клампы, мусорные имена, битый JSON, создание шапки. Прогонять после любой правки скрипта —
