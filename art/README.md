...
Настенные часы D2 теперь электронные: корпус/дисплей в `classroom_wall.svg`, живые цифры в GameplayWindow. Старые `clock_hand_*` сохранены как исходные ассеты, но больше не входят в раскладку/префаб Classroom. B3 уменьшает верхние плашки до 60 px (Unity `pixelsPerUnitMultiplier=1.5`), переносит SUSPICION в свободный промежуток (650,388) с масштабом 0.75, ограничивает пузырь областью (1140,378,260,112). D8-превью согласованы с этой раскладкой; Unity-проверки описаны в `../src/gamejam-template/tools/playtest/B3.md`.
